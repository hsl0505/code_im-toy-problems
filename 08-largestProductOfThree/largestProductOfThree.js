...
var largestProductOfThree = function(array) {
  ...
  if (count === array.length) { // 배열 안이 모두 음수일 경우 -> 값이 큰 순서로 정렬
  ...
  let copy = array.slice().sort(function(a,b) {  // 양수가 하나라도 있을 경우 절대값이 큰 순서로 정렬
  ...
  function recursion(arr) { // 처음 3개 -> 마지막 위치를 계속 이동시켜서 -> 없으면 그다음 2번째 위치를 이동시키고 다시 돌리고 
